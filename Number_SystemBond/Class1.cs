     ...
     public static class ConvertNumberSystem 
   {
       ...
       // private static string str1;
        ...
        private static float ConverTen(int n, string str)//перевод из любой в десятичную
        {
            ...
            if (number.Length < 3)
            {
                if (number.Length > 1)//проверка на дробную часть и на то, нет ли лишних знаков препинания
                {
                    try
                    {
        ...
        private static string ConvertOutTen(int n2)
        {
            ...
            string s2 = Convert.ToString((float)decimalNumber);// переводим в строку для разделения
